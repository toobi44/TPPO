...
char *MakeCRC(char *BitString)
{
   ...
   for (i=0; i<8; ++i)  CRC_[i] = 0;                    // Init before calculation
   
   for (i=0; i<strlen(BitString); ++i)
      {
      DoInvert = ('1'==BitString[i]) ^ CRC_[7];         // XOR required?
      ...
      CRC_[4] = CRC_[3];
      CRC_[3] = CRC_[2];
      CRC_[2] = CRC_[1] ^ DoInvert;
      CRC_[1] = CRC_[0] ^ DoInvert;
      CRC_[0] = DoInvert;
      }
      
   for (i=0; i<8; ++i)  Res[7-i] = CRC_[i] ? '1' : '0'; // Convert binary to ASCII
...
uint8_t Compute_CRC8(uint8_t bytes[], uint8_t Lenth)
{
    uint8_t crc = 0;
		...
		//uint8_t b = bytes;
    //foreach (uint8_t b in bytes)
    for (uint8_t i = 0; i < Lenth; i++)
		{
        /* XOR-in next input byte */
        //b = 
				data = (uint8_t)(bytes[i] ^ crc);
